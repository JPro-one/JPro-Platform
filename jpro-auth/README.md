...
2.0 specifications (the industry-standard protocol for authorization) and to some extent OpenID Connect Discovery, it
allows us to integrate with a wide range of identity providers, including Google, Microsoft, Keycloak and more. To
enhance performance and responsiveness, it supports asynchronous operations preventing UI blocking during authentication
processes.
...
- Comprehensive Identity Providers
...
1. Basic concepts
    ...
    - **Authentication Provider**: An authentication provider is a system or service that verifies and confirms the
      identity of users seeking to access an application or service. It validates user credentials, such as usernames
      and passwords or security tokens, to ensure that users are authenticated.

2. Advanced concepts
    - **Identity Provider**: A service  that authenticates users and issues credentials to third-party applications or
      websites, enabling users to access these services without the need to create new accounts. Examples of identity
      providers include Google, Microsoft, and Keycloak, among other OAuth2 providers.
    - **OAuth2**: An open standard for access delegation, widely utilized to allow internet users to grant websites or
      applications access to their information without sharing their passwords. It provides a secure and standardized
      method for authorization across different services.
...
3. Supported OAuth2 flows:
    - **Authorization Code Flow**: This is the most commonly used flow, where the user is redirected to the
      identity provider to authenticate and authorize the application. The authorization code grant type is used to
      obtain both access tokens and refresh tokens and is optimized for confidential clients. This flow is defined
      as part of the OAuth 2.0 specification in [RFC 6749, section 4.1](https://tools.ietf.org/html/rfc6749#section-4.1)
    - **Password Credentials Flow**: This flow is suitable in cases where the resource owner has a trust
      ...
      flows are not viable. This flow is defined as part of the OAuth 2.0 specification in [RFC 6749, section 4.3](https://tools.ietf.org/html/rfc6749#section-4.3)
    - **Client Credentials Flow**: This flow involves an application exchanging its application credentials, such
      ...
      the application instead of a user. This flow is defined as part of the OAuth 2.0 specification in
      [RFC 6749, section 4.4](https://tools.ietf.org/html/rfc6749#section-4.4)
    - **JWT Flow**: This flow is similar to the authorization code flow, but instead of returning an authorization
      code, the authorization server returns a signed JWT token. This flow is suitable for clients that cannot
      maintain the confidentiality of their client secret, such as native applications. This flow is defined as
      part of the OAuth 2.0 specification in [RFC 7523, section 2.1](https://tools.ietf.org/html/rfc7523#section-2.1)
...
4. Core Classes and Interfaces
    ...
    - The **UserManager** provides user management capabilities, such as creating, updating and deleting users. This
      interface should be implemented depending on the needs of the application and chosen solutions.
    - The **Credentials** interface is the base interface for all credentials. Currently, there are three
      implementations:
        ...
        3. **OAuth2Credentials**: This implementation holds the `accessToken`, `idToken` and `refreshToken` issued by an
           OAuth2 provider.
    - The **AuthenticationProvider** interface is the base interface for all authentication providers. Authentication is
      done asynchronously by calling the `authenticate` method with a given Credentials object, resulting in a User
      object. There are three major implementations:
        ...
        3. **OAuth2AuthenticationProvider**: This implementation can be used to authenticate users with an OAuth2 based
           ...
           OAuth2 provider.
    - **Option** classes are used to configure the authentication and authorization process, increasing the flexibility
      of the API.
...
- Gradle
    ```groovy
    dependencies {
          implementation("one.jpro.platform:jpro-auth:0.5.1-SNAPSHOT")
    }
    ```
- Maven
    ```xml
...
- Gradle
    ```groovy
    dependencies {
          implementation("one.jpro.platform:jpro-auth-routing:0.5.1-SNAPSHOT")
    }
    ```
- Maven
    ```xml
