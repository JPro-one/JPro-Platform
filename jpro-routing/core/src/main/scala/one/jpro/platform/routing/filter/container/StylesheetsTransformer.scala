 ...
 */
class StylesheetsTransformer(sheets: ObservableList[String]) extends ContainerTransformer {
  ...
  override def createNode(): Node = new StylesheetsTransformerContainer(sheets)
...
/** Internal wrapper Node for [[StylesheetsTransformer]]. The strong listener field lives
...
private final class StylesheetsTransformerContainer(source: ObservableList[String]) extends StackPane {
